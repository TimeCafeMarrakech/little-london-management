...
type EditCashbookExpensePageProps = {
  params: Promise<{ expenseId: string }>;
};

export default async function EditCashbookExpensePage({ params }: EditCashbookExpensePageProps) {
  const profile = await requireUserProfile();

  if (!canEditCashbookExpenses(profile)) {
    return <CashbookErrorState title="Access denied" message="You do not have permission to edit expenses." />;
  }
  ...
  if (expense.status !== "recorded" || expense.deletedAt) {
    return <CashbookErrorState title="Expense cannot be edited" message="Only recorded active expense entries can be edited." />;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[1.75rem] border border-[#eadfce] bg-[#fff8ee] p-6 shadow-[0_20px_55px_rgba(15,45,71,0.08)]">
        ...
        <h1 className="mt-2 text-3xl font-bold text-[#0f2d47]">Edit Expense</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-[#5b6f82]">Update an active recorded expense entry.</p>
      </section>
      <CashbookExpenseForm businessAreas={businessAreas} categories={categories} expense={expense} />
    </div>
  );
}
