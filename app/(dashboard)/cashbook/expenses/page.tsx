...
import { CashbookExpenseEmptyState } from "@/components/cashbook/cashbook-expense-empty-state";
import { CashbookExpenseFilters } from "@/components/cashbook/cashbook-expense-filters";
import { CashbookExpenseSummaryCards } from "@/components/cashbook/cashbook-expense-summary-cards";
import { CashbookExpenseTable } from "@/components/cashbook/cashbook-expense-table";
import { CashbookTabs } from "@/components/cashbook/cashbook-tabs";
import { Button } from "@/components/ui/button";
import { cashbookExpenseListQuerySchema } from "@/features/cashbook/schemas";
...
import {
  ...
  listCashbookExpenses,
} from "@/services/cashbook/cashbook-service";

export const dynamic = "force-dynamic";

type CashbookExpensesPageProps = {
...
export default async function CashbookExpensesPage({ searchParams }: CashbookExpensesPageProps) {
  ...
  if (!canViewCashbookExpenses(profile)) {
    return (
      <CashbookErrorState
        title="Access denied"
        message="Expenses are available to Super Admin and Admin users only. Teachers and parents do not have access to cashbook expenses."
      />
    );
  }

  const rawSearchParams = (await searchParams) ?? {};
  const filters = cashbookExpenseListQuerySchema.parse({
    ...
    expenseCategoryId: firstValue(rawSearchParams.expenseCategoryId),
  ...
  try {
    ...
    const canCreate = canCreateCashbookExpenses(profile);
    ...
    return (
      <div className="space-y-7">
        ...
        <CashbookExpenseSummaryCards summary={result.summary} />
        <CashbookExpenseFilters businessAreas={businessAreas} categories={categories} filters={filters} />

        {result.expenses.length > 0 ? (
          <CashbookExpenseTable canEdit={canEditCashbookExpenses(profile)} expenses={result.expenses} />
        ) : (
          <CashbookExpenseEmptyState canCreate={canCreate} />
        )}
      ...
      <CashbookErrorState
        message={
          error instanceof Error && error.message.includes("relation")
            ...
            : "Expenses could not be loaded. Please check the database connection and cashbook permissions."
        }
      />
    );
  }
