...
-- Phase 12: read-only reporting and analytics views.

insert into public.permissions (module, action, scope, key, description, is_system_permission)
values
  ('reports', 'view', 'all', 'reports.view.all', 'Management can view read-only operational reports.', true),
  ('reports', 'download', 'all', 'reports.download.all', 'Management can download approved reports.', true),
  ('reports', 'manage', 'all', 'reports.manage.all', 'Management can manage report definitions.', true)
...
with role_permission_keys(role_name, permission_key) as (
  values
    ...
    ('admin', 'reports.download.all'),
    ('admin', 'reports.export.all'),
    ('super_admin', 'reports.view.all'),
    ('super_admin', 'reports.download.all'),
    ('super_admin', 'reports.export.all'),
    ('super_admin', 'reports.manage.all')
)
