...
type BusinessDocumentActionsProps = {
  title: string;
  description: string;
  previewUrl: string;
  downloadUrl?: string;
...
export function BusinessDocumentActions({
  title,
  description,
  previewUrl,
  ...
  emailBody,
  whatsAppMessage,
  versionLabel = "Admin Version",
}: BusinessDocumentActionsProps) {
    ...
    <section className="overflow-hidden rounded-[1.5rem] border border-[#eadfce] bg-[#fff8ee] p-5 text-[#0f2d47] shadow-[0_22px_55px_rgba(15,45,71,0.08)]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[#5b6f82]">{description}</p>
        </div>
        <span className="w-fit rounded-full bg-[#8cc9a8]/20 px-3 py-1 text-xs font-semibold text-[#0f2d47]">{versionLabel}</span>
      </div>
