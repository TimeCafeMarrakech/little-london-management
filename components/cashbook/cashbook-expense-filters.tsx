...
import type { CashbookExpenseListQuery } from "@/features/cashbook/schemas";
import type { CashbookOption } from "@/features/cashbook/types";

type CashbookExpenseFiltersProps = {
  filters: CashbookExpenseListQuery;
  businessAreas: CashbookOption[];
  categories: CashbookOption[];
};

export function CashbookExpenseFilters({ filters, businessAreas, categories }: CashbookExpenseFiltersProps) {
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    ["query", "dateFrom", "dateTo", "businessAreaId", "expenseCategoryId", "paymentMethod", "status"].forEach((key) => {
    ...
    <form className="rounded-[1.35rem] border border-[#eadfce] bg-white/88 p-5 shadow-[0_16px_40px_rgba(15,45,71,0.07)]" onSubmit={onSubmit}>
      <div className="grid gap-3 lg:grid-cols-6 lg:items-end">
        ...
        <label className="block lg:col-span-2">
          ...
          <select className="mt-2 h-11 w-full rounded-2xl border border-[#dde5ec] bg-[#fffaf3] px-4 text-sm text-[#0f2d47] outline-none focus:border-[#f24a3a] focus:ring-2 focus:ring-[#f24a3a]/15" defaultValue={filters.expenseCategoryId} name="expenseCategoryId">
